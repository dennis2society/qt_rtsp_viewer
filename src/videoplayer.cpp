...
VideoPlayer::VideoPlayer(QWidget *parent)
    : QWidget(parent), mediaPlayer(nullptr), videoWidget(nullptr), statusLabel(nullptr),
      captureSink(nullptr), frameUpdateTimer(nullptr), frameCount(0), currentFps(0.0)
{
    setupUI();
...
void VideoPlayer::paintFPSOverlay(QImage &image, const QString &fpsText) {
    ...
    QString resolutionText = QString("%1x%2")
                                 .arg(image.width())
                                 .arg(image.height());

    QString overlayText = QString("%1 | %2")
                              .arg(fpsText)
                              .arg(resolutionText);
    ...
    textRect.moveTopRight(
        QPoint(image.width() - margin, textRect.height())
    );
