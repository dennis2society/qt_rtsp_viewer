...
VideoEffects::VideoEffects(QObject *parent)
    : QObject(parent), blurAmount(0), grayscaleEnabled(false),
      sepiaEnabled(false), brightnessAmount(0), contrastAmount(0)
{
}
...
QImage VideoEffects::applyEffects(const QImage &sourceImage)
{
    ...
    if (blurAmount == 0 && !grayscaleEnabled && !sepiaEnabled && 
        brightnessAmount == 0 && contrastAmount == 0) {
        return sourceImage;
    }
